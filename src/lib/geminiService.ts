...
function resolveGeminiModelName(model?: string): string {
  if (!model) return 'gemini-3.7-flash-preview';
  const clean = model.replace('models/', '');
  if (clean.includes('pro')) return 'gemini-3.1-pro-preview';
  if (clean.includes('lite')) return 'gemini-3.0-flash-lite';
  return 'gemini-3.7-flash-preview';
}
 ...
 */
export async function rollSingleFieldAI(category: ExperienceCategory, fieldType: 'title' | 'heroName' | 'roleClass' | 'raceOrigin' | 'hook', model = 'gemini-3.7-flash-preview'): Promise<string> {
  const apiKey = getStoredApiKey();
  const targetModel = resolveGeminiModelName(model);
