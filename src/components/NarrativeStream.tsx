...
export const NarrativeStream: React.FC<NarrativeStreamProps> = ({
  ...
  useEffect(() => {
    // Only scroll the internal messages feed when a new message has actually been added
    if (messagesContainerRef.current && logs.length > prevLogsCountRef.current) {
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-190px)] sm:h-[600px] lg:h-[720px] rounded-lg bg-[#111118] border border-white/10 shadow-2xl overflow-hidden text-slate-200">
      
      {/* Stream Header */}
      <div className="px-3 sm:px-4 py-2 sm:py-2.5 bg-[#16161D] border-b border-white/10 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="p-1 sm:p-1.5 rounded bg-white/5 text-amber-400 border border-white/10 shrink-0">
            {getCategoryIcon(experienceCategory, "w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-400")}
          </div>
          <div className="min-w-0">
            <h3 className="text-xs font-serif font-bold text-amber-50 truncate">Narrative Experience Log</h3>
            <p className="text-[8px] sm:text-[9px] text-slate-400 font-mono uppercase tracking-wider truncate">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {pendingCheck && (
            <span className="px-1.5 sm:px-2 py-0.5 rounded bg-red-950/80 border border-red-600/50 text-[9px] sm:text-[10px] font-mono text-red-300 font-bold flex items-center gap-1 animate-pulse">
              <Lock className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-red-400" />
              <span className="hidden xs:inline">DC {pendingCheck.dc} Check</span>
              <span className="xs:hidden">DC {pendingCheck.dc}</span>
            </span>
          )}

          <span className="px-1.5 sm:px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[9px] sm:text-[10px] font-mono text-amber-300 flex items-center gap-1 max-w-[120px] sm:max-w-none truncate">
      ...
      {/* Messages Feed */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#0A0A0F]">
        {logs.map((log) => {
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1.5 pb-0.5">
                    {log.suggestedActions.map((sa, idx) => (
                      <button
                        ...
                        className="text-[11px] sm:text-xs bg-white/5 hover:bg-amber-500/10 active:bg-amber-500/20 text-amber-300 hover:text-amber-200 border border-white/10 hover:border-amber-500/50 px-2.5 sm:px-3 py-1.5 rounded-lg transition text-left shadow-sm flex items-center gap-1.5 active:scale-95"
                      >
      ...
      <div className="p-3 bg-[#16161D] border-t border-white/10 space-y-2">
        ...
        {pendingCheck ? (
          ...
          /* Normal Action Bar */
          <div className="relative flex items-end sm:items-center gap-1.5 sm:gap-2">
            <textarea
              rows={2}
              value={inputText}
              ...
              className="flex-1 bg-[#0A0A0F] border border-white/10 rounded-lg p-2.5 text-xs sm:text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-amber-600/50 leading-relaxed resize-none min-h-[52px]"
            />
            ...
            <div className="flex flex-col gap-1 shrink-0">
              <button
                ...
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-amber-600 hover:bg-amber-500 active:scale-95 disabled:opacity-40 text-black font-bold transition shadow flex items-center justify-center"
                title="Send Turn"
              >
              ...
              <button
                ...
                disabled={!inputText.trim() || isGenerating}
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-purple-900/80 hover:bg-purple-800 active:scale-95 disabled:opacity-40 text-purple-200 border border-purple-700/50 transition flex items-center justify-center"
