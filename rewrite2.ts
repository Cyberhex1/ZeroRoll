...
content = content.replace("const ai = new GoogleGenAI({ apiKey });", "const vertexAI = getVertexAI(app);\n      const aiModel = getGenerativeModel(vertexAI, { model: targetModel, generationConfig: { temperature: 0.9 }, systemInstruction: promptInstruction });");
...
content = content.replace("let text = response.text || '{}';", "let text = response.response.text() || '{}';");
content = content.replace("console.warn('Gemini Scenario generation error:', err);", "console.warn('Vertex AI Scenario generation error:', err);");
content = content.replace(/}\n\n  \/\/ Fallback to high-entropy procedural generator if NO key provided/g, "}  // Fallback to high-entropy procedural generator if Vertex AI fails");
...
content = content.replace("const text = response.text || '{}';", "const text = response.response.text() || '{}';");
content = content.replace("console.warn('Gemini Seedlist generation error:', err);", "console.warn('Vertex AI Seedlist generation error:', err);");
content = content.replace(/}\n\n  \/\/ Fallback to dynamic procedural seedlist if NO key provided/g, "}  // Fallback to dynamic procedural seedlist if Vertex AI fails");
...
content = content.replace("console.warn('Gemini Roll Field error:', err);", "console.warn('Vertex AI Roll Field error:', err);");
content = content.replace(/}\n\n  \/\/ Fallback if NO key provided/g, "}  // Fallback if Vertex AI fails");
...
content = content.replace("const text = res.text?.trim() || '';", "const text = res.response.text()?.trim() || '';");
content = content.replace("console.warn('Gemini Avatar generation error:', err);", "console.warn('Vertex AI Avatar generation error:', err);");
content = content.replace(/}\n\n  \/\/ Beautiful geometric avatar fallback if NO key provided/g, "}  // Beautiful geometric avatar fallback if Vertex AI fails");
